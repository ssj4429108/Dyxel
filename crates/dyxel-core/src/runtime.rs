    ...
    match opcode {
        ...
        OpCode::RegisterRotationHandler => {
            if payload.len() >= 4 {
                ...
                get_handler_registry()
                    .lock()
                    .unwrap()
                    .register(id, HandlerType::Rotation);
            }
        }
        OpCode::RegisterPointerDownHandler => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                log::info!("RegisterPointerDownHandler for node {}", id);
                get_handler_registry()
                    .lock()
                    .unwrap()
                    .register(id, HandlerType::PointerDown);
            }
        }
        OpCode::RegisterPointerUpHandler => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                log::info!("RegisterPointerUpHandler for node {}", id);
                get_handler_registry()
                    .lock()
                    .unwrap()
                    .register(id, HandlerType::PointerUp);
            }
        }
        ...
        OpCode::RegisterTapHandlerV2 => {
            if payload.len() >= 7 {
                ...
                log::info!(
                    "RegisterTapHandlerV2 for node {}: count={}, gap={}ms",
                    id,
                    count,
                    multi_click_gap_ms
                );
                get_handler_registry().lock().unwrap().set_tap_config(
                    id,
                    count,
                    multi_click_gap_ms,
                );
        ...
        OpCode::RegisterLongPressHandlerV2 => {
            if payload.len() >= 6 {
                ...
                log::info!(
                    "RegisterLongPressHandlerV2 for node {}: timeout={}ms, slop={}",
                    id,
                    timeout_ms,
                    slop
                );
                get_handler_registry()
                    .lock()
                    .unwrap()
                    .set_long_press_config(id, timeout_ms, slop);
            }
        }
        OpCode::RegisterPanHandlerV2 => {
            if payload.len() >= 6 {
                ...
                log::info!(
                    "RegisterPanHandlerV2 for node {}: slop={}, direction={}",
                    id,
                    slop,
                    direction
                );
                get_handler_registry()
                    .lock()
                    .unwrap()
                    .set_pan_config(id, slop, direction);
            }
        }
        ...
        OpCode::SetBorderWidth => {
            if payload.len() >= 8 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let width = f32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]);
                state.set_border_width(id, width);
            }
        }
        OpCode::SetBorderColor => {
            if payload.len() >= 8 {
                ...
                let b = payload[6];
                let a = payload[7];
                state.set_border_color(id, r, g, b, a);
            }
        }
        ...
        OpCode::CreateTextInput => {
            if payload.len() >= 4 {
                ...
                crate::text_input::create_text_input(id);
            }
        }
        OpCode::SetTextInputFocused => {
            if payload.len() >= 5 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let focused = payload[4] != 0;
                crate::text_input::set_focused(id, focused);
            }
        }
        OpCode::SyncTextState => {
            if payload.len() >= 16 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let len =
                    u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]) as usize;
                let start = u32::from_le_bytes([payload[8], payload[9], payload[10], payload[11]]);
                let end = u32::from_le_bytes([payload[12], payload[13], payload[14], payload[15]]);
        ...
        OpCode::SetTextInputCursor => {
            if payload.len() >= 8 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let pos = u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]);
                crate::text_input::set_cursor_position(id, pos);
            }
        }
        OpCode::SetTextInputSelection => {
            if payload.len() >= 12 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let start = u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]);
                let end = u32::from_le_bytes([payload[8], payload[9], payload[10], payload[11]]);
                crate::text_input::set_selection(id, start, end);
            }
        }
        OpCode::SetTextInputPlaceholder => {
            if payload.len() >= 8 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let len =
                ...
                if text_end <= payload.len() {
                    let text = String::from_utf8_lossy(&payload[text_start..text_end]).to_string();
                    crate::text_input::set_placeholder(id, text);
                }
            }
        }
        OpCode::SetTextInputType => {
            if payload.len() >= 5 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let input_type = dyxel_shared::InputType::from_u8(payload[4]);
                crate::text_input::set_input_type(id, input_type);
            }
        }
        OpCode::SetTextInputReturnKeyType => {
            if payload.len() >= 5 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let key_type = dyxel_shared::ReturnKeyType::from_u8(payload[4]);
                crate::text_input::set_return_key_type(id, key_type);
            }
        }
        OpCode::SetTextInputMaxLength => {
            if payload.len() >= 8 {
                let wasm_id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let id = state.resolve_id(wasm_id);
                let max_len = u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]);
                crate::text_input::set_max_length(id, max_len);
            }
        }
        OpCode::SetTextInputSecure => {
            if payload.len() >= 5 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let secure = payload[4] != 0;
                crate::text_input::set_secure(id, secure);
            }
        }
        OpCode::ShowTextInputKeyboard => {
            crate::text_input::show_keyboard();
        }
        OpCode::HideTextInputKeyboard => {
            crate::text_input::hide_keyboard();
        }
        OpCode::CopyToClipboard => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                crate::text_input::copy(id);
            }
        }
        OpCode::CutToClipboard => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                crate::text_input::cut(id);
            }
        }
        OpCode::RequestPasteFromClipboard => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                crate::text_input::request_paste(id);
            }
        }
        OpCode::ShowTextInputContextMenu => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                log::debug!("Show context menu for text input {}", id);
                // Show context menu (position will be determined by platform integration)
                crate::text_input::show_context_menu(id, None);
            }
        }
        OpCode::HideTextInputContextMenu => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                log::debug!("Hide context menu for text input {}", id);
                crate::text_input::hide_context_menu(id);
            }
        }
        OpCode::TextInputMenuItemSelected => {
            if payload.len() >= 5 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let item = payload[4];
                log::debug!("TextInput menu item {} selected for node {}", item, id);
                // Handle menu item selection using the new handler
                crate::text_input::handle_menu_item(id, item);
            }
        }
        OpCode::SetComposingText => {
            if payload.len() >= 8 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let len =
                    u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]) as usize;
                log::debug!("Set composing text for node {} (len={})", id, len);
                // TODO: Read actual text from string buffer
                crate::text_input::TextInputManager::with(|m| {
                    if let Some(state) = m.get_mut(id) {
                        // Placeholder: set composing text
                        state.composing_text = String::new();
                    }
                });
            }
        }
        OpCode::CommitComposingText => {
            if payload.len() >= 4 {
                let id = u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                log::debug!("Commit composing text for node {}", id);
                crate::text_input::TextInputManager::with(|m| {
                    if let Some(state) = m.get_mut(id) {
                        state.commit_composing_text();
                    }
                });
            }
        }
        OpCode::KeyboardWillShow => {
            if payload.len() >= 9 {
                let height = f32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let animation_duration_ms =
                    u32::from_le_bytes([payload[4], payload[5], payload[6], payload[7]]);
                let _animation_curve = payload[8];
                log::debug!(
                    "Keyboard will show: height={}, duration={}ms",
                    height,
                    animation_duration_ms
                );
                crate::keyboard::update_keyboard_state(true, height, animation_duration_ms);
            }
        }
        OpCode::KeyboardWillHide => {
            if payload.len() >= 5 {
                let animation_duration_ms =
                    u32::from_le_bytes([payload[0], payload[1], payload[2], payload[3]]);
                let _animation_curve = payload[4];
                log::debug!("Keyboard will hide: duration={}ms", animation_duration_ms);
                crate::keyboard::update_keyboard_state(false, 0.0, animation_duration_ms);
            }
        }
