...
fn test_layout_registry_opcodes_handled() {
    ...
    for op in layout_ops {
        ...
        assert_eq!(
            dirty,
            DirtyField::None,
            "{:?} should not produce dirty fields",
            op
        );
    }
}
...
fn test_layout_read_after_write_pattern() {
    ...
    assert!(
        tracker.is_node_dirty(10),
        "Node 10 should be dirty after sync"
    );
    ...
    assert!(
        !tracker.is_node_dirty(10),
        "Node 10 should be clean after clear"
    );
}
...
fn test_batch_layout_operations() {
    ...
    for i in 0..100 {
        assert!(
            !tracker.is_node_dirty(i),
            "Node {} should be clean after clear",
            i
        );
    }
}
