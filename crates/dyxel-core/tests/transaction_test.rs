...
fn test_transaction_basic() {
    ...
    // Begin transaction
    tx.begin(1, 0).unwrap();
    assert!(matches!(tx.state, TransactionState::Active { seq_id: 1, .. }));
    ...
    assert!(matches!(tx.state, TransactionState::Committed { seq_id: 1 }));
...
fn test_dirty_field_combinations() {
    ...
    assert!(has_field(fields, DirtyField::Style), "Style should still be set");
    assert!(has_field(fields, DirtyField::Size), "Size should now be set");
    
    // Mark Layout - should combine with existing fields
    tracker.mark_dirty(0, DirtyField::Layout);
    let fields = tracker.get_dirty_fields(0);
    assert!(has_field(fields, DirtyField::Style), "Style should still be set");
    assert!(has_field(fields, DirtyField::Size), "Size should still be set");
    assert!(has_field(fields, DirtyField::Layout), "Layout should now be set");
}

#[test]
fn test_get_dirty_field_for_opcode() {
    assert_eq!(get_dirty_field_for_opcode(&OpCode::SetColor), DirtyField::Style);
    assert_eq!(get_dirty_field_for_opcode(&OpCode::SetWidth), DirtyField::Size);
    assert_eq!(get_dirty_field_for_opcode(&OpCode::SetFlexDirection), DirtyField::Layout);
    assert_eq!(get_dirty_field_for_opcode(&OpCode::SetText), DirtyField::Text);
    assert_eq!(get_dirty_field_for_opcode(&OpCode::AddChild), DirtyField::Children);
}
...
fn test_same_frame_multiple_updates_renders_last() {
    ...
    assert_eq!(cmds.len(), 2, "Same node+field should deduplicate to single command per field");
    
    // Find the color command and verify it has the LAST value
    let color_cmd = cmds.iter().find(|c| c.opcode == OpCode::SetColor).unwrap();
    assert_eq!(color_cmd.payload[4], 128, "Should keep last color value (gray R)");
    assert_eq!(color_cmd.payload[5], 128, "Should keep last color value (gray G)");
    assert_eq!(color_cmd.payload[6], 128, "Should keep last color value (gray B)");
    
    // Find the width command and verify it has the LAST value (40)
    let width_cmd = cmds.iter().find(|c| c.opcode == OpCode::SetWidth).unwrap();
    assert_eq!(width_cmd.payload[5], 40, "Should keep last width value (4 * 10 = 40)");
}
...
fn test_no_tearing_background_and_position_sync() {
    ...
    assert!(has_field(fields, DirtyField::Style), "Style should be dirty");
    assert!(has_field(fields, DirtyField::Size), "Size should be dirty");
    assert!(has_field(fields, DirtyField::Layout), "Layout should be dirty");
...
fn test_add_child_never_deduplicated() {
    ...
    assert_eq!(cmds.len(), 5, "AddChild commands should never be deduplicated");
}
