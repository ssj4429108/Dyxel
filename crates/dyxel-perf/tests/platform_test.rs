...
use dyxel_perf::{PerfConfig, PerformanceMonitor};
use std::thread;
...
fn test_macos_memory() {
    ...
    assert!(
        final_stats.total_frames >= 5,
        "Should have tracked at least 5 frames"
    );

    // Memory should be non-zero on macOS
    if monitor.platform_name() == "macos" {
        assert!(
            final_stats.memory_used_mb > 0.0,
            "Memory usage should be > 0 on macOS"
        );
...
fn test_overlay_toggle() {
    ...
    assert!(
        ...
        "Overlay should be disabled after toggle"
    );

    monitor.toggle_overlay();
    assert!(
        monitor.should_show_overlay(),
        "Overlay should be enabled after second toggle"
    );
