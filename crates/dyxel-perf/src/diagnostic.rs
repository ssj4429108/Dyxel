...
impl PerformanceDiagnostics {
    ...
    pub fn end_frame(&mut self, gpu_time_ms: f32, wasm_time_ms: f32, layout_time_ms: f32) {
        let now = Instant::now();
        let total_time_ms = self.last_frame_start
    ...
    pub fn analyze_bottleneck(&self) -> BottleneckAnalysis {
        ...
        let avg_total: f32 = self.frames.iter().map(|f| f.total_time_ms).sum::<f32>() / self.frames.len() as f32;
        let avg_gpu: f32 = self.frames.iter().map(|f| f.gpu_time_ms).sum::<f32>() / self.frames.len() as f32;
        let avg_wasm: f32 = self.frames.iter().map(|f| f.wasm_time_ms).sum::<f32>() / self.frames.len() as f32;
        let avg_layout: f32 = self.frames.iter().map(|f| f.layout_time_ms).sum::<f32>() / self.frames.len() as f32;
