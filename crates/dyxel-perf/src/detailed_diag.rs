...
mod tests {
    ...
    fn test_frame_timer() {
        ...
        assert!(
            logic_time >= 1.5 && logic_time < 5.0,
            "Logic time should be ~2ms"
        );
        assert!(
            render_time >= 2.5 && render_time < 6.0,
            "Render time should be ~3ms"
        );
