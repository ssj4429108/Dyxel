...
use dyxel_shared::DirtyTracker;
use crate::texture_pool::{GpuTexturePool};
use dyxel_render_api::TextureId;
// use vello::Scene;
...
impl RasterCache {
    ...
    pub fn update_stability(&mut self, dirty_tracker: &dyxel_shared::DirtyTracker) -> Vec<u32> {
    ...
    pub fn recycle_unused(&mut self, pool: &mut GpuTexturePool) -> usize {
        let mut recycled = 0;
    ...
    pub fn check_memory_pressure(&mut self, pool: &mut GpuTexturePool) -> usize {
        ...
        let mut entries_by_access: Vec<_> = self
            ...
            .map(|(id, e)| (*id, e.last_access_frame))
            .collect();

        entries_by_access.sort_by_key(|(_, frame)| *frame);

        let mut evicted = 0;
        let mut memory_freed = 0;
        let target_free = self.stats.memory_used - budget_bytes;

        for (node_id, _) in entries_by_access {
            if memory_freed >= target_free {
                break;
            }

            if let Some(entry) = self.entries.get(&node_id) {
                let entry_memory = (entry.texture_size.0 * entry.texture_size.1 * 4) as usize;
