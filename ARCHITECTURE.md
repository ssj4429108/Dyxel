...
### 3.4 内存优化系统 (Memory Optimization)
Dyxel 实现了一套分层内存配置系统，根据设备 RAM 容量自动调整资源使用：

| 设备等级 | RAM 范围 | Vello 缓冲区 | 纹理图集 | 字体缓存 | WASM 初始内存 |
|---------|---------|-------------|---------|---------|--------------|
| 低端机 | <4GB | 35% | 1024 | 32MB | 8MB (128页) |
| 中端机 | 4-6GB | 60% | 2048 | 64MB | 16MB (256页) |
| 高端机 | 8GB+ | 100% | 4096 | 128MB | 32MB (512页) |

- **检测方式**: Android 读取 `/proc/meminfo`，macOS 默认为高端，Web/其他平台默认为中端
- **实现位置**: `dyxel-perf/src/memory_optimizer.rs` - `MemoryOptimizer` 结构体
- **使用方式**: Vello 渲染器在初始化时检测并应用对应的纹理格式和缓冲区大小
